...
$(document.querySelector('#rightColumn > div.right-tab-header')).append("<a id="
PriorityLabel
" title = "
KGAutoPlay:\
nLow
...
function autoTrade() {
    ...
    if((gamePage.religion.getRU('solarRevolution').val == 1 || (gamePage.resPool.get('gold').value == gamePage.resPool.get('gold').maxValue && gamePage.resPool.get('gold').maxValue < 500)) || (gamePage.ironWill)) {
        ...
        if((goldResource.value > goldResource.maxValue * 0.95) || (gamePage.ironWill && goldResource.value > 600)) {
            ...
            let trade = tradersAll.filter(tr => gamePage.diplomacy.get(tr[0]).unlocked && tr[1].value < tr[1].maxValue * tr[3] && tr[2].value > tr[1].value && tr[2].value > tr[4]).sort(function (a, b) {
                return a[1].value / a[1].maxValue - b[1].value / b[1].maxValue;
            })[0]
            if(trade) {
        ...
        if(!gamePage.ironWill && (cntcrafts == 0 || cntcrafts >= 200 || (Object.keys(craftPriority[0]).length > 0 && craftPriority[2] != gamePage.bld.getBuildingExt(craftPriority[0]).meta.val))) {
            ...
            prior = prior.sort(function (a, b) {
                ...
                return (Object.keys(a[2]).reduce(function (c, d) {
                    ...
                    return c + s
                }, 0) / a[0] - Object.keys(b[2]).reduce(function (c, d) {
                    var s = 1
...
function autoAssign() {
    ...
    if(gamePage.science.get('civil').researched && !gamePage.ironWill) {
        if(IincKAssign > 100) {
            let prkitten = gamePage.village.sim.kittens.filter(kitten => kitten.job == restmpq[0][1]).sort(function (a, b) {
                return b.skills[restmpq[0][1]] - a.skills[restmpq[0][1]];
            })[0]
            if(prkitten) {
